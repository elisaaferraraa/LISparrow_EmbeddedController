...
void AgileCascadedPID::setAttitudeReference(float roll_rad_ref, float pitch_rad_ref, float roll_rad, float vel_u)
{
    ...
    yaw_dot = 9.81f * std::tan(-roll_rad) / vel_u; // body forward velocity
...
void AgileCascadedPID::setThrustReference(float vel_ref, float vel_u)
{
    thrust_ref_ = pid_thrust_.update(-(vel_ref - vel_u), dt_);
}
    ...
    float u_rud)
{
    ...
    if (q_err.w() < 0.0f)
        q_err.coeffs() *= -1.0f; // Ensure shortest rotation
    ...
    actuator_outputs_[0] = clamp(thrust_ref_, -0.9f, 1.0f);           // thrust
